/**
 * This file was generated by the Objective Systems ASN1C Compiler
 * (http://www.obj-sys.com).  Version: 7.5.0, Date: 07-May-2021.
 */
package org.mobicents.gmlc.slee.lpp.LPP_PDU_Definitions;

import com.objsys.asn1j.runtime.*;

public class ResultsPerSSB_Index_r16 extends Asn1Seq {
   private static final long serialVersionUID = 55;
   static {
      _setKey (_LPP_PDU_DefinitionsRtkey._rtkey);
      Asn1Type._setLicLocation(_LPP_PDU_DefinitionsRtkey._licLocation);
   }

   public String getAsn1TypeName()  {
      return "ResultsPerSSB-Index-r16";
   }

   protected Asn1Integer ssb_Index_r16;
   protected MeasQuantityResults_r16 ssb_Results_r16;

   public ResultsPerSSB_Index_r16 () {
      super();
      init();
   }

   /**
    * This constructor sets all elements to references to the 
    * given objects
    */
   public ResultsPerSSB_Index_r16 (
      Asn1Integer ssb_Index_r16_,
      MeasQuantityResults_r16 ssb_Results_r16_
   ) throws Asn1Exception {
      super();
      setSsb_Index_r16 (ssb_Index_r16_);
      setSsb_Results_r16 (ssb_Results_r16_);
   }

   /**
    * This constructor allows primitive data to be passed for all 
    * primitive elements.  It will create new object wrappers for 
    * the primitive data and set other elements to references to 
    * the given objects 
    */
   public ResultsPerSSB_Index_r16 (long ssb_Index_r16_,
      MeasQuantityResults_r16 ssb_Results_r16_
   ) throws Asn1Exception {
      super();
      setSsb_Index_r16 (ssb_Index_r16_);
      setSsb_Results_r16 (ssb_Results_r16_);
   }

   public void init () {
      ssb_Index_r16 = null;
      ssb_Results_r16 = null;
   }

   public boolean equals( Object obj ) {
      if ( this == obj ) return true;
      if ( !(obj instanceof ResultsPerSSB_Index_r16) ) return false;

      ResultsPerSSB_Index_r16 rhs = (ResultsPerSSB_Index_r16) obj;

      if (ssb_Index_r16 == null) {
         if (rhs.ssb_Index_r16 != null) return false;
      }
      else {
         if (!ssb_Index_r16.equals(rhs.ssb_Index_r16)) {
            return false;
         }
      }

      if (ssb_Results_r16 == null) {
         if (rhs.ssb_Results_r16 != null) return false;
      }
      else {
         if (!ssb_Results_r16.equals(rhs.ssb_Results_r16)) {
            return false;
         }
      }

      return true;
   }

   public int hashCode() {
      int __code = 1;

      if (ssb_Index_r16 != null) __code = 31*__code + ssb_Index_r16.hashCode();
      if (ssb_Results_r16 != null) __code = 31*__code + ssb_Results_r16.hashCode();

      return __code;
   }

   /**
    * Accessor/mutator methods for Ssb_Index_r16
    */
   public Asn1Integer getSsb_Index_r16 () {
      return ssb_Index_r16;
   }

   public void setSsb_Index_r16 (Asn1Integer value) 
      throws Asn1Exception
   {
      if (!((value.value >= 0 && value.value <= 63))) {
         throw new Asn1ConsVioException ("value.value", value.value);
      }

      this.ssb_Index_r16 = value;
   }

   public void setSsb_Index_r16 (long value) 
      throws Asn1Exception
   {
      setSsb_Index_r16 (new Asn1Integer(value));
   }

   /**
    * Accessor/mutator methods for Ssb_Results_r16
    */
   public MeasQuantityResults_r16 getSsb_Results_r16 () {
      return ssb_Results_r16;
   }

   public void setSsb_Results_r16 (MeasQuantityResults_r16 value) {
      this.ssb_Results_r16 = value;
   }

   public int getElementCount() { return 2; }


   public Object getElementValue(int index){
      switch(index)  {
         case 0: return ssb_Index_r16;
         case 1: return ssb_Results_r16;
         default: return null;
      }
   }


   public String getElementName(int index){
      switch(index)  {
         case 0: return "ssb-Index-r16";
         case 1: return "ssb-Results-r16";
         default: return null;
      }
   }


   public void decode (Asn1PerDecodeBuffer buffer)
      throws Asn1Exception, java.io.IOException
   {
      init ();

      // decode ssb_Index_r16

      buffer.getContext().eventDispatcher.startElement("ssb_Index_r16", -1);

      ssb_Index_r16 = new Asn1Integer();
      ssb_Index_r16.decode (buffer, 0, 63);
      buffer.getContext().eventDispatcher.endElement("ssb_Index_r16", -1);

      // decode ssb_Results_r16

      buffer.getContext().eventDispatcher.startElement("ssb_Results_r16", -1);

      ssb_Results_r16 = new MeasQuantityResults_r16();
      ssb_Results_r16.decode (buffer);
      buffer.getContext().eventDispatcher.endElement("ssb_Results_r16", -1);

   }

   public void encode (Asn1PerEncodeBuffer buffer)
      throws Asn1Exception, java.io.IOException
   {
      // encode ssb_Index_r16

      if (ssb_Index_r16 != null) {
         buffer.getContext().eventDispatcher.startElement("ssb_Index_r16", -1);

         ssb_Index_r16.encode (buffer, 0, 63);

         buffer.getContext().eventDispatcher.endElement("ssb_Index_r16", -1);
      }
      else throw new Asn1MissingRequiredException ("ssb_Index_r16");

      // encode ssb_Results_r16

      if (ssb_Results_r16 != null) {
         buffer.getContext().eventDispatcher.startElement("ssb_Results_r16", -1);

         ssb_Results_r16.encode (buffer);

         buffer.getContext().eventDispatcher.endElement("ssb_Results_r16", -1);
      }
      else throw new Asn1MissingRequiredException ("ssb_Results_r16");

   }

}
